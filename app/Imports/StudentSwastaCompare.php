...
class StudentSwastaCompare implements ToModel, WithValidation, WithHeadingRow
{
    ...
    public function model(array $row)
    {
        ...
        // if (!isset($row['Nama Pelajar']) || !isset($row['Nama Penjaga']) || !isset($row['Jantina Pelajar']) || !isset($row['IC Penjaga']) || !isset($row['Email Penjaga'])) {
        //     if (empty($row['Nama Pelajar']) && empty($row['Nama Penjaga']) && empty($row['Jantina Pelajar']) && empty($row['IC Penjaga']) && empty($row['Email Penjaga'])) {
        //         return null;
        //     } else {
        //         throw ValidationException::withMessages(["error" => "Invalid headers or missing column"]);
        //     }
        ...
        if (!filter_var($parentEmail, FILTER_VALIDATE_EMAIL)) {
            throw ValidationException::withMessages(["error" => "Invalid email format"]);
        }
        ...
        $findStudent=DB::table('students as s')
                            ...
                            ->where('u.telno',$phone)
                            ->select('s.id as studentId','s.nama as studentName','s.gender as gender','u.name as parentName','u.id as parentId','u.telno as parentTelno', 'u.email as parentEmail')
                            ->first();
        ...
        if($findStudent!=null){
            ...
            if(count($sameOrgStudent)>0)
            {
                ...
                if(count($sameClassStudent)>0){
                    ...
                    $findStudent->parentEmail = $parentEmail;
            ...
            $newStudentData->gender = $row["jantina_pelajar"];
            $newStudentData->parentName =$parentName  ;
            $newStudentData->parentTelno = $phone;
            $newStudentData->parentEmail = $parentEmail;
